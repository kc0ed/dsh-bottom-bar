...
// range 可带 days(JS getDay:周日0…周六6),缺省 = 每天
export const PEAK_RANGES = [
  { start: 9 * 60, end: 12 * 60, days: [1, 2, 3, 4, 5] },
  { start: 14 * 60, end: 18 * 60, days: [1, 2, 3, 4, 5] },
]
export const PEAK_RANGE_LABELS = ['工作日 9:00-12:00', '工作日 14:00-18:00']
// ── 修订 191：周末计费调整(官方公告)──
// 北京时间 2026-08-23 00:00 起,周六日全天不再分峰谷、统一按低谷价;
// 该时刻之前仍按旧规则(每天 9:00-12:00 / 14:00-18:00 均为高峰)判定。
// peakRangesFor(when):默认窗口的时代切换——历史调用按其发生时刻选规则,
// 自定义窗口(用户自设 peakWindow)不参与切换。
export const PEAK_RANGES_LEGACY = [
  { start: 9 * 60, end: 12 * 60 },
  { start: 14 * 60, end: 18 * 60 },
]
export const WEEKEND_IDLE_START = new Date('2026-08-23T00:00:00+08:00')
export function peakRangesFor(when) {
  const t = (when === undefined || when === null ? new Date() : when).getTime()
  return Number.isFinite(t) && t < WEEKEND_IDLE_START.getTime() ? PEAK_RANGES_LEGACY : PEAK_RANGES
}
...
export const DEFAULT_PRICES = {
  ...
  // 修订 191：官方新模型(视觉实验版),价格与 flash 完全一致
...
// 修订 191：目标时区的星期(JS getDay:周日0…周六6)——UTC±N 需按偏移平移日期
export function zoneDayOfWeek(date, tz) {
  const d = date === undefined || date === null ? new Date() : date
  if (tz === undefined || tz === null || tz === 'system') return d.getDay()
  if (tz === 'UTC') return d.getUTCDay()
  const m = /^UTC([+-])(\d{1,2})$/.exec(String(tz))
  if (m === null) return d.getDay()
  const offset = (m[1] === '-' ? -1 : 1) * Number(m[2])
  return new Date(d.getTime() + offset * 3600000).getUTCDay()
}
export function isPeakTime(date, tz, ranges) {
  const m = zoneMinutes(date, tz)
  const dow = zoneDayOfWeek(date, tz)
  const rs = Array.isArray(ranges) && ranges.length > 0 ? ranges : PEAK_RANGES
  // 修订 191：range 可带 days(缺省每天);星期不匹配即使时刻命中也不算高峰
  return rs.some((r) => m >= r.start && m < r.end && (r.days === undefined || r.days.includes(dow)))
}
...
export function resolvePrice(model, configPrices, opts) {
  ...
  if (user === undefined) {
    ...
    // 修订 140：附加字段一并合并(否则用户自定义价丢 peak/alt/ctxScales/peakWindow)
    merged = {
      ...
      peak: user.peak !== undefined ? user.peak : base.peak,
      alt: user.alt !== undefined ? user.alt : base.alt,
      ctxScales: user.ctxScales !== undefined ? user.ctxScales : base.ctxScales,
      peakWindow: user.peakWindow !== undefined ? user.peakWindow : base.peakWindow,
      // 修订 174：固定比例开关(用户条目)
      lock: user.lock === true,
    }
  }
  ...
  // 修订 140：上下文长度分档——条目可带 ctxScales: [{ limit, price: {in, cacheRead,
  // cacheWrite, out, peak?} }, ...](按 limit 升序);请求上下文量 opts.ctx 超过某档
  // limit 即用该档价(命中最后一档);峰值随后叠在当前档之上(档内可带自己的 peak)。
  if (opts !== undefined && typeof opts.ctx === 'number' && Number.isFinite(opts.ctx) && opts.ctx > 0) {
    const scales = merged.ctxScales !== undefined ? merged.ctxScales : (base !== undefined ? base.ctxScales : undefined)
    if (Array.isArray(scales) && scales.length > 0) {
      let pick = undefined
      let pickLimit = undefined
      for (const s of scales) {
        if (s !== null && s !== undefined && typeof s.limit === 'number' && opts.ctx > s.limit) {
          pick = s.price
          pickLimit = s.limit
        }
      }
      if (pick !== null && pick !== undefined && typeof pick === 'object') {
        merged = {
          currency: pick.currency !== undefined ? pick.currency : merged.currency,
          in: pick.in !== undefined ? pick.in : merged.in,
          cacheRead: pick.cacheRead !== undefined ? pick.cacheRead : merged.cacheRead,
          cacheWrite: pick.cacheWrite !== undefined ? pick.cacheWrite : merged.cacheWrite,
          out: pick.out !== undefined ? pick.out : merged.out,
          alt: merged.alt,
          peak: pick.peak !== undefined ? pick.peak : (merged.peak !== undefined ? merged.peak : base !== undefined ? base.peak : undefined),
          ctxScale: pickLimit,
        }
      }
    }
  ...
  // 修订 191:默认窗口(内置声明或缺省)按 08-23 时代切换;用户自定义窗口原样尊重
  let peakWindow = peakWindowOf(merged)
  if (peakWindow === PEAK_RANGES || peakWindow === PEAK_RANGES_LEGACY) peakWindow = peakRangesFor(opts !== undefined ? opts.when : undefined)
...
export function buildPriceList(configPrices) {
  ...
  for (const model of builtin.concat(extra)) {
    ...
    rows.push({
      ...
      // 修订 140：该行是否带上下文长度分档
      hasScales: Array.isArray(p.ctxScales) && p.ctxScales.length > 0,
      // 修订 121：行内峰谷编辑——输出 peak 值供设置页行内展开编辑
      peak: p.peak !== undefined
        ? {
            in: p.peak.in !== undefined ? p.peak.in : p.in,
            cacheRead: p.peak.cacheRead !== undefined ? p.peak.cacheRead : (p.cacheRead !== undefined ? p.cacheRead : null),
            cacheWrite: p.peak.cacheWrite !== undefined ? p.peak.cacheWrite : (p.cacheWrite !== undefined ? p.cacheWrite : null),
            out: p.peak.out !== undefined ? p.peak.out : p.out,
            locked: p.peak.locked === true,
          }
        : null,
      // 修订 174：固定比例开关(弹层编辑持久化)
      lock: p.lock === true,
      // 修订 183：双币种卡——输出 alt(官方双币价)供弹层费率卡展示
      alt: p.alt !== undefined && p.alt !== null && typeof p.alt === 'object'
        ? {
            currency: p.alt.currency === 'CNY' ? 'CNY' : 'USD',
            in: Number(p.alt.in) || 0,
            cacheRead: p.alt.cacheRead !== undefined ? p.alt.cacheRead : null,
            cacheWrite: p.alt.cacheWrite !== undefined ? p.alt.cacheWrite : null,
            out: Number(p.alt.out) || 0,
            peak: p.alt.peak !== undefined && p.alt.peak !== null && typeof p.alt.peak === 'object'
              ? {
                  in: Number(p.alt.peak.in) || 0,
                  cacheRead: p.alt.peak.cacheRead !== undefined ? p.alt.peak.cacheRead : null,
                  cacheWrite: p.alt.peak.cacheWrite !== undefined ? p.alt.peak.cacheWrite : null,
                  out: Number(p.alt.peak.out) || 0,
                }
              : null,
          }
        : null,
