...
export const DEFAULT_PRICES = {
  ...
  // 修订 75：v4-flash / v4-pro 自 2026-08-17 00:00（北京时间）起实行峰谷定价——
  // 高峰时段 9:00-12:00、14:00-18:00（北京时间），空闲时段价格 = 高峰的一半。
  // 基价 = 新空闲价，peak = 高峰价组（缓存写官方无收费项 → 0）。
  'deepseek-v4-flash': { currency: 'CNY', in: 1.5, cacheRead: 0.05, cacheWrite: 0, out: 4.5, peak: { currency: 'CNY', in: 3.0, cacheRead: 0.10, cacheWrite: 0, out: 9.0 } },
  'deepseek-v4-pro': { currency: 'CNY', in: 4.5, cacheRead: 0.15, cacheWrite: 0, out: 13.5, peak: { currency: 'CNY', in: 9.0, cacheRead: 0.30, cacheWrite: 0, out: 27.0 } },
...
// ── 峰谷定价时段（DeepSeek 2026-08-17 起;时段 9:00-12:00 / 14:00-18:00）
...
export function isPeakTime(date, tz) {
  const m = zoneMinutes(date, tz)
  return PEAK_RANGES.some((r) => m >= r.start && m < r.end)
}
// 修订 92：当前所在的高峰窗口（标签或 null=空闲时段）
export const PEAK_RANGE_LABELS = ['9:00-12:00', '14:00-18:00']
export function currentPeakRangeLabel(date, tz) {
  const m = zoneMinutes(date, tz)
  const i = PEAK_RANGES.findIndex((r) => m >= r.start && m < r.end)
  return i === -1 ? null : PEAK_RANGE_LABELS[i]
}

export function derivePriceByFamily(modelId, inPrice) {
  const m = modelId.toLowerCase()
  // DeepSeek 系: CNY, v4 空闲价 out 3x / read 1/30 / write 0;v3 系 out 4x / read 0.25x / write 1.0x
  if (m.includes('deepseek')) {
    const isV4 = m.includes('v4') || m.includes('flash')
    const outMult = isV4 ? 3 : 4
    const readMult = isV4 ? 0.0333 : 0.25
    return {
      currency: 'CNY',
      in: inPrice,
...
export function resolvePrice(model, configPrices, opts) {
  ...
  if (user === undefined && base === undefined) {
    ...
    if (m.includes('kimi') || m.includes('moonshot')) {
      ...
      base = { currency: 'CNY', in: 1.5, cacheRead: 0.05, cacheWrite: 0, out: 4.5 }
  ...
  // 峰谷定价:开关开启、渠道在白名单（修订 76:仅 DeepSeek 官方 provider
  // 'deepseek',三方网关如 opencode-go 未确认透传官方峰谷价前不适用）、
  // 当前处于高峰时段 → 用 peak 价组覆盖
  ...
  if (opts !== undefined && opts.peakEnabled === true && providersOk && peakSet !== undefined && isPeakTime(opts.when, opts.tz)) {
    merged = {
