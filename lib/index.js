...
class BottomBarService extends TypertRemoteService {
  ...
  applyUsage(state, usage, turn, step) {
    ...
    state.lastUsage = { turn, step, model: key, provider: state.lastProvider, bucket: bucketName, ...buckets }
  }
  ...
  async reconcileWithProjection(sessionId, state, liveSession, clientUsage) {
    ...
    const dst = bucketName === 'peak' ? row.peak : row.idle
    dst.uncachedInput += gap.uncachedInput
    dst.output += gap.output
    dst.cacheRead += gap.cacheRead
    dst.cacheWrite += gap.cacheWrite
