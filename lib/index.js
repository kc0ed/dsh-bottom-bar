...
class BottomBarService extends TypertRemoteService {
  ...
  priceOf(model) {
    return resolvePrice(model, this.configPrices, { peakEnabled: this.peakEnabled, peakProviders: this.peakProviders, when: new Date() })
  }
  ...
  async saveConfig(segments, mode, tooltip, precision, prices, peakEnabled, peakProviders) {
  ...
  snapshot() {
    return {
      ...
      peakEnabled: this.peakEnabled,
      peakProviders: this.peakProviders,
      peakNow: isPeakTime(new Date()) ? 'peak' : 'offpeak',
  ...
  async estimateCost(args) {
    ...
    const peakNow = isPeakTime(new Date()) ? 'peak' : 'offpeak'
    ...
    try {
      ...
      est.peak = { enabled: this.peakEnabled && anyOfficial, window: peakNow }
      return est
    } catch (err) {
  ...
  async getClientUsage() {
    ...
    const peakInfo = { enabled: this.peakEnabled && officialProvider, window: peakNow }
    if (price === undefined) {
  ...
  async diagnostics() {
    return {
      ...
      peakEnabled: this.peakEnabled,
      peakProviders: this.peakProviders,
      peakWindow: isPeakTime(new Date()) ? 'peak' : 'offpeak',
    }
  }
  ...
  async setConfig(args) {
    ...
    await this.saveConfig(normalized !== null ? normalized : undefined, a.mode, a.tooltip, a.precision, undefined, a.peakEnabled, a.peakProviders)
    return this.snapshot()
  }
