      ...
      if (live !== undefined && live !== null) {
        try {
          ...
          if (nodes.length > seen) {
            const rec = st.recent === undefined ? { items: [], sumMs: 0, sumOut: 0, sumTtft: 0, ttftN: 0 } : st.recent
            for (let i = seen; i < nodes.length; i++) {
              ...
              rec.items.push({ ms: decodeMs, out: outTokens })
              rec.sumMs += decodeMs
          ...
          est.recent = st.recent !== undefined && st.recent.items.length > 0
            ? { count: st.recent.items.length, decodeMs: st.recent.sumMs, outputTokens: st.recent.sumOut, ttftMs: st.recent.ttftN > 0 ? st.recent.sumTtft / st.recent.ttftN : null }
            : null
