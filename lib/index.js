...
class BottomBarService extends TypertRemoteService {
  ...
  async saveConfig(segments, mode, tooltip, precision, prices, peakEnabled, peakProviders, peakTimezone) {
  ...
  async estimateCost(args) {
    ...
    try {
      ...
      est.peak = { enabled: this.peakEnabled && anyOfficial, window: peakNow, tz: this.peakTimezone }
      return est
    } catch (err) {
  ...
  async getClientUsage() {
    ...
    const peakInfo = { enabled: this.peakEnabled && officialProvider, window: peakNow, tz: this.peakTimezone }
    if (price === undefined) {
  ...
  async setConfig(args) {
    ...
    await this.saveConfig(normalized !== null ? normalized : undefined, a.mode, a.tooltip, a.precision, undefined, a.peakEnabled, a.peakProviders, a.peakTimezone)
    return this.snapshot()
  }
