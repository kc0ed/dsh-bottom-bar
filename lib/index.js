...
import { balanceMixin } from './dsk-balance.js'
import {
  DEFAULT_PRICES,
  resolvePrice,
  buildPriceList,
...
// 1.1.1 拆出:余额相关方法 → lib/dsk-balance.js(balanceMixin,挂原型)
...
class BottomBarService extends TypertRemoteService {
  constructor(ctx, config) {
    ...
    // 修订 144/146：厂商比例四格编排顺序(默认 A = 输入:输出:缓存读:缓存写,
    // 与 models.dev 官方数据对表一致;备选 B = 输入:缓存读:缓存写:输出;
    // 仅展示排列,不影响乘数换算)
    this.ratioOrder = 'in-out-read-write'
    // 修订 159：峰谷提示语可自定义(默认 高峰/低谷,预设 梁文峰/梁文谷),
    // 随 est.peak.labelOn/labelOff 带给底栏与明细面板
    this.peakLabel = '高峰'
    this.peakIdleLabel = '低谷'
    // 修订 168：DeepSeek Platform 余额查询——key 只存本地配置,不进仓库;
    // 底栏「余额」分段 + getBalance RPC(可强制刷新),缓存 5 分钟。
    // 修订 187：自动读取 DSH 官方 key(~/.dsh/.credentials.yaml 的
    // DEEPSEEK_API_KEY)作兜底,手动 dskKey 优先;快照暴露 dskKeyAuto
    this.dskKey = ''
    this.dskKeyAuto = ''
    this.balanceCache = { t: 0, data: null }
    this.balanceBusy = false
    this.dskCredLoaded = false
  }
  ...
  // ── 修订 190：峰谷分时账本(双桶) ──
  // 行结构 v2:{ idle: {uncachedInput,output,cacheRead,cacheWrite}, peak: {同} }
  // idle = 平价时段累计(含 2026-08-17 峰谷制生效前的全部历史);peak = 高峰窗口内。
  // 费用 = idle×平价 + peak×峰价 —— 真实账单模拟,不再随查看时刻翻转。
  flatZero() {
    return { uncachedInput: 0, output: 0, cacheRead: 0, cacheWrite: 0 }
  }

  rowZero() {
    return { idle: this.flatZero(), peak: this.flatZero() }
  }

  rowFlat(row) {
    const i = row.idle || this.flatZero()
    const p = row.peak || this.flatZero()
    return {
      uncachedInput: (i.uncachedInput || 0) + (p.uncachedInput || 0),
      output: (i.output || 0) + (p.output || 0),
      cacheRead: (i.cacheRead || 0) + (p.cacheRead || 0),
      cacheWrite: (i.cacheWrite || 0) + (p.cacheWrite || 0),
    }
  }

  // 该模型的新增用量进哪个桶:价目带峰谷 → 按当前时刻判窗口;否则永远平价桶
  pickBucket(modelKey) {
    try {
      const p = resolvePrice(modelKey, this.configPrices, { peakEnabled: false })
      if (p !== undefined && p.peak !== undefined) {
        let ranges = peakWindowOf(p)
        // 修订 191:默认窗口按 08-23 周末计费调整做时代切换
        if (ranges === PEAK_RANGES || ranges === PEAK_RANGES_LEGACY) ranges = peakRangesFor(new Date())
        return isPeakTime(new Date(), this.peakTimezone, ranges) ? 'peak' : 'idle'
      }
    } catch (err) { /* ignore */ }
    return 'idle'
  }

  // 双桶计价:idle×平价 + peak×峰价(峰价缺项回落平价);返回 null = 无价目
  dualCostOf(modelKey, idleF, peakF, ctx) {
    const price = resolvePrice(modelKey, this.configPrices, { peakEnabled: false, ...(typeof ctx === 'number' && Number.isFinite(ctx) && ctx > 0 ? { ctx } : {}) })
    if (price === undefined) return null
    const currency = price.currency || 'USD'
    const pk = price.peak
    const pkRate = (base, k) => (pk !== undefined && pk[k] !== undefined ? pk[k] : base)
    const costOf = (f, usePeak) => usePeak
      ? f.uncachedInput / 1e6 * pkRate(price.in, 'in')
        + f.cacheRead / 1e6 * pkRate(price.cacheRead ?? price.in, 'cacheRead')
        + f.cacheWrite / 1e6 * pkRate(price.cacheWrite ?? price.in, 'cacheWrite')
        + f.output / 1e6 * pkRate(price.out, 'out')
      : f.uncachedInput / 1e6 * (price.in ?? 0)
        + f.cacheRead / 1e6 * (price.cacheRead ?? price.in ?? 0)
        + f.cacheWrite / 1e6 * (price.cacheWrite ?? price.in ?? 0)
        + f.output / 1e6 * (price.out ?? 0)
    const idleCost = costOf(idleF, false)
    const peakCost = costOf(peakF, true)
    return { cost: idleCost + peakCost, currency, idleCost, peakCost, price }
  }

  applyUsage(state, usage, turn, step, route) {
    const effectiveProvider = route !== undefined && route !== null ? (typeof route.provider === 'string' ? route.provider : null) : state.lastProvider
    const effectiveModel = route !== undefined && route !== null ? route.model : state.lastModel
    const rowKey = this.rowKeyOf(effectiveProvider, effectiveModel)
    const buckets = {
    ...
    if (prev !== null) {
      ...
      if (oldRow !== undefined) {
        // 修订 190:从 prev 当时落入的桶里扣回(替换语义跨桶安全)
        const pb = prev.bucket === 'peak' ? oldRow.peak : (oldRow.idle !== undefined ? oldRow.idle : oldRow)
        pb.uncachedInput -= prev.uncachedInput
        pb.output -= prev.output
        pb.cacheRead -= prev.cacheRead
        pb.cacheWrite -= prev.cacheWrite
      }
    }
    let row = state.models.get(key)
    if (row === undefined || row.idle === undefined) {
      row = this.rowZero()
      state.models.set(key, row)
    }
    // 修订 190:带峰谷价的模型按当前时刻进 idle/peak 桶;其余永远平价桶
    const bucketName = this.pickBucket(key)
    const dst = bucketName === 'peak' ? row.peak : row.idle
    dst.uncachedInput += buckets.uncachedInput
    dst.output += buckets.output
    dst.cacheRead += buckets.cacheRead
    dst.cacheWrite += buckets.cacheWrite
    state.lastUsage = { turn, step, model: key, provider: effectiveProvider, bucket: bucketName, ...buckets }
    if (route !== undefined && route !== null) {
      state.lastProvider = effectiveProvider
      state.lastModel = effectiveModel
    }
  }
  ...
  foldEvent(state, event) {
    ...
    let turn
    let step
    // 修订 193:累计账本只采纳最终 assistant/message;chunk 仅是流式临时数据
    let route
    if (event.type === 'assistant/message' && event.data.usage !== undefined) {
      ...
      if (src !== undefined && src !== null && typeof src.model === 'string' && src.model.length > 0) {
        route = { model: src.model, provider: typeof src.provider === 'string' && src.provider.length > 0 ? src.provider : null }
      }
    }
    if (usage !== undefined) this.applyUsage(state, usage, turn, step, route)
  }
  ...
  healAttribution(state, provider, model) {
    ...
    if (q !== undefined) {
      ...
      if (row === undefined) {
        // 修订 190:双桶行整体接管
        state.models.set(key, row === undefined ? q : row)
      } else {
        for (const bk of ['idle', 'peak']) {
          const dst = row[bk] !== undefined ? row[bk] : (row[bk] = this.flatZero())
          const src = q[bk] !== undefined ? q[bk] : q
          dst.uncachedInput += src.uncachedInput || 0
    ...
    for (const [sid, entry] of Object.entries(this.ledger.sessions)) {
      ...
      if (typeof entry.models === 'object' && entry.models !== null) {
        for (const [model, b] of Object.entries(entry.models)) {
          // 修订 190:v1 平铺结构自动迁移为双桶(idle=原量,peak=0);v2 直读
          let row
          if (b !== null && typeof b === 'object' && b.idle !== undefined && typeof b.idle === 'object') {
            row = {
              idle: {
                uncachedInput: Number(b.idle.uncachedInput) || 0,
                output: Number(b.idle.output) || 0,
                cacheRead: Number(b.idle.cacheRead) || 0,
                cacheWrite: Number(b.idle.cacheWrite) || 0,
              },
              peak: {
                uncachedInput: Number(b.peak !== undefined && b.peak !== null ? b.peak.uncachedInput : 0) || 0,
                output: Number(b.peak !== undefined && b.peak !== null ? b.peak.output : 0) || 0,
                cacheRead: Number(b.peak !== undefined && b.peak !== null ? b.peak.cacheRead : 0) || 0,
                cacheWrite: Number(b.peak !== undefined && b.peak !== null ? b.peak.cacheWrite : 0) || 0,
              },
            }
          } else {
            row = {
          ...
          models.set(this.normalizeKey(model), row)
        }
      }
      this.liveStates.set(sid, {
        ...
        unattributedGap: entry.unattributedGap !== undefined && entry.unattributedGap !== null ? entry.unattributedGap : null,
      })
    }
  }
  ...
  persistSession(sessionId) {
    ...
    for (const [model, b] of st.models) {
      // 修订 190:双桶持久化(v2 结构)
      const bi = b.idle !== undefined ? b.idle : b
      const bp = b.peak !== undefined ? b.peak : this.flatZero()
      models[model] = {
        idle: { uncachedInput: bi.uncachedInput, output: bi.output, cacheRead: bi.cacheRead, cacheWrite: bi.cacheWrite },
        peak: { uncachedInput: bp.uncachedInput, output: bp.output, cacheRead: bp.cacheRead, cacheWrite: bp.cacheWrite },
      }
    }
    this.ledger.sessions[sessionId] = {
      ...
      unattributedGap: st.unattributedGap || null,
      models,
    }
  ...
  priceOf(model, ctx) {
    return resolvePrice(model, this.configPrices, { peakEnabled: this.peakEnabled, peakProviders: this.peakProviders, tz: this.peakTimezone, when: new Date(), ...(typeof ctx === 'number' && Number.isFinite(ctx) && ctx > 0 ? { ctx } : {}) })
  }
  ...
  estimateOf(state) {
    ...
    const totals = {}
    let anyTokens = false
    // 修订 140：代表上下文量 = 最近一次请求的输入总量(估档用)
    const repCtx = (() => {
      const lu = state.lastUsage
      return lu !== null && lu !== undefined ? (lu.uncachedInput || 0) + (lu.cacheRead || 0) + (lu.cacheWrite || 0) : undefined
    })()
    for (const [model, row] of state.models) {
      // 修订 190:双桶行——展示用平铺视图,计费 idle×平价 + peak×峰价
      const u = this.rowFlat(row)
      const tokens = u.uncachedInput + u.output + u.cacheRead + u.cacheWrite
      if (tokens > 0) anyTokens = true
      const dual = this.dualCostOf(model, row.idle || this.flatZero(), row.peak || this.flatZero(), repCtx)
      if (dual === null) {
        unpriced.push({ model, uncachedInput: u.uncachedInput, output: u.output, cacheRead: u.cacheRead, cacheWrite: u.cacheWrite })
        continue
      }
      const price = dual.price
      const currency = price.currency || 'USD'
      const cost = dual.cost
      ...
      // 修订 190:费用加权峰占比(0~1)与峰/谷分项成本
      const peakShare = cost > 0 ? dual.peakCost / cost : 0
      // 新增内容(去复读):未缓存输入 + 输出 —— 每次调用真正新注入/新产出的,
      // 复读(cacheRead/cacheWrite)是以前见过的内容;compaction 后缓存失效重注
      // 会使新增略高估(但厂商确实按未命中收费,账单口径无误)
      const freshTokens = u.uncachedInput + u.output
      const rereadTokens = u.cacheRead + u.cacheWrite
      priced.push({
        ...
        freshTokens, rereadTokens,
        peakShare, idleCost: dual.idleCost, peakCost: dual.peakCost,
        priceIn: price.in,
        ...
        priceOut: price.out,
        inCost: u.uncachedInput / 1e6 * (price.in ?? 0),
        cacheReadCost: u.cacheRead / 1e6 * (price.cacheRead ?? price.in ?? 0),
        cacheWriteCost: u.cacheWrite / 1e6 * (price.cacheWrite ?? price.in ?? 0),
        outCost: u.output / 1e6 * (price.out ?? 0),
      })
    }
  ...
  async reconcileWithProjection(sessionId, state, liveSession, clientUsage) {
    ...
    for (const b of state.models.values()) {
      const flat = this.rowFlat(b)
      total.uncachedInput += flat.uncachedInput
      total.output += flat.output
      total.cacheRead += flat.cacheRead
      total.cacheWrite += flat.cacheWrite
    }
    const gap = {
    ...
    // 修订 193:projection 只有会话总量,没有可靠模型归属;缺口不塞入任何模型。
    state.unattributedGap = { ...gap, at: now }
  ...
  normalizePrices(raw) {
    ...
    for (const model of Object.keys(raw)) {
      ...
      // 修订 182：重启不再丢自定义峰谷/分档/alt/窗口——此前 normalizePrices
      // 只留五个字段,peak/ctxScales/alt/peakWindow 一重启就没了
      if (p.peak !== undefined && p.peak !== null && typeof p.peak === 'object') {
        entry.peak = {
          ...
          locked: p.peak.locked === true,
        }
      }
      if (Array.isArray(p.ctxScales) && p.ctxScales.length > 0) entry.ctxScales = p.ctxScales
      if (p.alt !== undefined && p.alt !== null && typeof p.alt === 'object') entry.alt = p.alt
      if (p.peakWindow !== undefined && p.peakWindow !== null) entry.peakWindow = p.peakWindow
      out[model] = entry
    }
    return out
  }
  ...
  async loadConfig() {
    // 修订 187：自动读取 DSH 官方 key(只读一次,dskCredLoaded 防重复)
    await this.readDskKeyAuto()
    ...
    if (parsed !== null && typeof parsed === 'object') {
      ...
      if (normalized !== null) {
        ...
        if (parsed.ratioOrder === 'in-read-write-out' || parsed.ratioOrder === 'in-out-read-write') this.ratioOrder = parsed.ratioOrder
        if (typeof parsed.peakLabel === 'string' && parsed.peakLabel !== '') this.peakLabel = parsed.peakLabel
        if (typeof parsed.peakIdleLabel === 'string' && parsed.peakIdleLabel !== '') this.peakIdleLabel = parsed.peakIdleLabel
        if (typeof parsed.dskKey === 'string') this.dskKey = parsed.dskKey
        await this.readDskKeyAuto()
  ...
  async saveConfig(segments, mode, tooltip, precision, prices, peakEnabled, peakProviders, peakTimezone, peakRemind, ratioOrder, peakLabel, peakIdleLabel, dskKey) {
    ...
    if (ratioOrder === 'in-read-write-out' || ratioOrder === 'in-out-read-write') this.ratioOrder = ratioOrder
    if (typeof peakLabel === 'string' && peakLabel !== '') this.peakLabel = peakLabel
    if (typeof peakIdleLabel === 'string' && peakIdleLabel !== '') this.peakIdleLabel = peakIdleLabel
    if (typeof dskKey === 'string') this.dskKey = dskKey
    ...
    const ok = await this.writeJsonFile(this.configPath(), {
      ...
      ratioOrder: this.ratioOrder,
      peakLabel: this.peakLabel,
      peakIdleLabel: this.peakIdleLabel,
      dskKey: this.dskKey,
  ...
  snapshot() {
    return {
      ...
      peakIdleLabel: this.peakIdleLabel,
      dskKey: this.dskKey,
      dskKeyAuto: this.dskKeyAuto !== '',
      peakNow: isPeakTime(new Date(), this.peakTimezone) ? 'peak' : 'offpeak',
      persisted: this.compositionPersisted,
    }
  }

  // 1.1.1 拆出:余额方法(readDskKeyAuto/dskKeyEffective/balanceSnapshot/
  // pullBalance/fetchDskBalance/fetchDskBalanceNode/parseDskBalance/getBalance)
  // → lib/dsk-balance.js balanceMixin(本类原型末尾 Object.assign 挂载)
  priceList() {
    ...
    if (sessions !== null && sessions !== undefined) {
      for (const sid of Object.keys(sessions)) {
        ...
        for (const key of Object.keys(m)) {
          const u = m[key]
          // 修订 190:v2 双桶结构拆 idle/peak 聚合;v1 平铺视为全 idle
          const isV2 = u !== null && typeof u === 'object' && u.idle !== undefined && typeof u.idle === 'object'
          const iF = isV2 ? u.idle : u
          const pF = isV2 ? (u.peak || {}) : {}
          const tokens = ((iF.uncachedInput || 0) + (iF.output || 0) + (iF.cacheRead || 0) + (iF.cacheWrite || 0))
            + ((pF.uncachedInput || 0) + (pF.output || 0) + (pF.cacheRead || 0) + (pF.cacheWrite || 0))
          if (tokens <= 0) continue
          let cur = agg.get(key)
          if (cur === undefined) {
            cur = { key, tokens: 0, uncachedInput: 0, output: 0, cacheRead: 0, cacheWrite: 0, idle: this.flatZero(), peak: this.flatZero() }
            agg.set(key, cur)
          }
          cur.tokens += tokens
          // 平铺视图 = idle + peak;peak 桶单独累计(供双桶计价)
          for (const src of [iF, pF]) {
            cur.uncachedInput += src.uncachedInput || 0
            cur.output += src.output || 0
            cur.cacheRead += src.cacheRead || 0
            cur.cacheWrite += src.cacheWrite || 0
          }
          for (const k of ['uncachedInput', 'output', 'cacheRead', 'cacheWrite']) {
            cur.peak[k] += pF[k] || 0
          }
        }
      }
  ...
  async estimateCost(args) {
    ...
    try {
        ...
        try {
          ...
          if (nodes.length > seen) {
            ...
            for (let i = seen; i < nodes.length; i++) {
              ...
              // 修订 133：字段兜底——opencode 等渠道节点 usage 可能无
              // uncachedInputTokens(只有 inputTokens),取不到就当 0 会误显示「未缓存输入0」
              const uncachedTokens = u === undefined || u === null ? 0
                : (u.uncachedInputTokens !== undefined ? u.uncachedInputTokens : (u.inputTokens !== undefined ? u.inputTokens : 0))
              ...
              const inTokens = uncachedTokens + readTokens + writeTokens
      ...
      // 修订 191:默认窗口按 08-23 周末计费调整做时代切换
      let repRanges = peakRangesFor(new Date())
      let repLabels = windowLabelsOf(repRanges)
      let peakNow = isPeakTime(new Date(), this.peakTimezone, repRanges) ? 'peak' : 'offpeak'
      if (priceModel !== undefined) {
        const baseP2 = resolvePrice(priceModel.model, this.configPrices, { peakEnabled: false })
        if (baseP2 !== undefined) {
          repRanges = peakWindowOf(baseP2)
          if (repRanges === PEAK_RANGES || repRanges === PEAK_RANGES_LEGACY) repRanges = peakRangesFor(new Date())
          repLabels = windowLabelsOf(repRanges)
          peakNow = isPeakTime(new Date(), this.peakTimezone, repRanges) ? 'peak' : 'offpeak'
        }
      }
      est.peak = { enabled: this.peakRemind, priced: this.peakEnabled && anyOfficial, billing: this.peakEnabled, window: peakNow, activeRange: currentPeakRangeLabel(new Date(), this.peakTimezone, repRanges, repLabels), tz: this.peakTimezone, providers: this.peakProviders, ranges: repLabels, baseIn, baseRead, baseOut, peakIn, peakRead, peakOut, models: this.peakModelList(), labelOn: this.peakLabel, labelOff: this.peakIdleLabel }
      // 修订 190:费用加权峰占比(0~1)——底栏「· 峰xx%」;分时计费后总额不随查看时刻翻转
      let shareCost = 0
      let sharePeak = 0
      for (const p of est.priced) {
        shareCost += p.cost
        sharePeak += p.peakCost || 0
      }
      est.peakShare = shareCost > 0 ? sharePeak / shareCost : 0
      // 修订 168：底栏「余额」分段——余额快照(缓存 5 分钟,无 key 返回 nokey)
      est.balance = this.balanceSnapshot()
      return est
    } catch (err) {
  ...
  summaryAll() {
    ...
    for (const u of used) {
      ...
      // 修订 190:双桶计价——idle×平价 + peak×峰价
      const peakF = u.peak || this.flatZero()
      const idleF = {
        uncachedInput: (u.uncachedInput || 0) - (peakF.uncachedInput || 0),
        output: (u.output || 0) - (peakF.output || 0),
        cacheRead: (u.cacheRead || 0) - (peakF.cacheRead || 0),
        cacheWrite: (u.cacheWrite || 0) - (peakF.cacheWrite || 0),
      }
      const dual = this.dualCostOf(u.key, idleF, peakF, (u.uncachedInput || 0) + (u.cacheRead || 0) + (u.cacheWrite || 0))
      const price = dual !== null ? dual.price : undefined
      let cost = dual !== null ? dual.cost : 0
      let currency = dual !== null ? dual.currency : 'USD'
      totals[currency] = (totals[currency] || 0) + cost
      ...
      // 修订 115：分桶消耗/费用(分享卡按模型细化用);190:附峰谷分项
      ...
      const peakShare = cost > 0 && dual !== null ? dual.peakCost / cost : 0
      // 新增内容(去复读):未缓存输入 + 输出;复读 = 缓存读 + 缓存写
      const freshTokens = (u.uncachedInput || 0) + (u.output || 0)
      const rereadTokens = (u.cacheRead || 0) + (u.cacheWrite || 0)
      rows.push({ key: u.key, provider: u.provider, model: u.model, tokens: u.tokens, cost, currency, source, free: isFree, refCost, refCurrency, uncachedInput: u.uncachedInput, cacheRead: u.cacheRead, cacheWrite: u.cacheWrite, output: u.output, inCost, cacheReadCost, cacheWriteCost, outCost, peakShare, freshTokens, rereadTokens, idleCost: dual !== null ? dual.idleCost : 0, peakCost: dual !== null ? dual.peakCost : 0 })
    }
  ...
  async getClientUsage() {
    ...
    // 修订 140：客户端全量卡计价也按代表上下文估档
    const usageCtx = (this.lastClientUsage.uncachedInput || 0) + (this.lastClientUsage.cacheRead || 0) + (this.lastClientUsage.cacheWrite || 0)
    const price = this.priceOf(modelKey, usageCtx)
    const peakNow = isPeakTime(new Date(), this.peakTimezone) ? 'peak' : 'offpeak'
    ...
    if (price === undefined) {
      return { usage: { at: this.lastClientUsage.at, model: modelKey, provider: this.lastClientUsage.provider, currency: 'USD', uncachedInput: this.lastClientUsage.uncachedInput, cacheRead: this.lastClientUsage.cacheRead, cacheWrite: this.lastClientUsage.cacheWrite, output: this.lastClientUsage.output, inCost: null, cacheReadCost: null, cacheWriteCost: null, outCost: null, total: null, peak: peakInfo }, all: this.summaryAll(), balance: this.balanceSnapshot() }
    }
  ...
  async setConfig(args) {
    ...
    await this.saveConfig(normalized !== null ? normalized : undefined, a.mode, a.tooltip, a.precision, undefined, a.peakEnabled, a.peakProviders, a.peakTimezone, a.peakRemind, a.ratioOrder, a.peakLabel, a.peakIdleLabel, a.dskKey)
    return this.snapshot()
  }
  ...
  async setPrice(args) {
    ...
    if (isBlank) {
      ...
      this.configPrices[a.model] = {
        ...
        peak: ref !== undefined && ref.peak !== undefined
          ? {
              ...
              locked: ref.peak.locked === true,
            }
          : undefined,
        // 修订 140：一键配置带官方长度分档
        ctxScales: ref !== undefined && Array.isArray(ref.ctxScales) && ref.ctxScales.length > 0 ? ref.ctxScales : undefined,
      }
    } else {
      ...
      this.configPrices[a.model] = {
        ...
        // 修订 140：上下文长度分档透传(每档 {limit, price{...}})
        ctxScales: p.ctxScales !== undefined && p.ctxScales !== null ? p.ctxScales : (prev !== undefined && prev.ctxScales !== undefined ? prev.ctxScales : undefined),
        // 修订 174：固定比例开关(显式布尔透传,未传保留旧值)
        lock: typeof p.lock === 'boolean' ? p.lock : (prev !== undefined && prev.lock === true),
      }
    }
...
// 1.1.1:余额方法挂载到原型(REMOTE_METHODS 的 marker 在构造器里逐名调用,
// 方法本体来自 mixin 同样在原型上,行为不变)
Object.assign(BottomBarService.prototype, balanceMixin)
