...
class BottomBarService extends TypertRemoteService {
  ...
  async estimateCost(args) {
    ...
    try {
      ...
      // 修订 82：取第一个官方渠道模型的高峰/空闲价,供分段与详情展示
      const officialKey = est.priced.find(officialIn) || est.unpriced.find(officialIn)
      let baseIn = null, baseOut = null, peakIn = null, peakOut = null
      if (officialKey !== undefined) {
        const baseP = resolvePrice(officialKey.model, this.configPrices, { peakEnabled: false })
        if (baseP !== undefined) {
          baseIn = baseP.in
          baseOut = baseP.out
          const pk = baseP.peak
