...
class BottomBarService extends TypertRemoteService {
  ...
  priceOf(model) {
    return resolvePrice(model, this.configPrices, { peakEnabled: this.peakEnabled, when: new Date() })
  }
  ...
  async saveConfig(segments, mode, tooltip, precision, prices, peakEnabled) {
  ...
  async estimateCost(args) {
    ...
    try {
      ...
      est.peak = { enabled: this.peakEnabled, window: peakNow }
      return est
    } catch (err) {
  ...
  async getClientUsage() {
    ...
    const peakNow = isPeakTime(new Date()) ? 'peak' : 'offpeak'
    const peakInfo = { enabled: this.peakEnabled, window: peakNow }
    if (price === undefined) {
  ...
  async setConfig(args) {
    ...
    await this.saveConfig(normalized !== null ? normalized : undefined, a.mode, a.tooltip, a.precision, undefined, a.peakEnabled)
    return this.snapshot()
  }
