...
class BottomBarService extends TypertRemoteService {
  ...
  async estimateCost(args) {
    ...
    try {
      ...
      est.peak = { enabled: this.peakRemind, priced: this.peakEnabled && anyOfficial, billing: this.peakEnabled, window: peakNow, activeRange: currentPeakRangeLabel(new Date(), this.peakTimezone), tz: this.peakTimezone, baseIn, baseRead, baseOut, peakIn, peakRead, peakOut, models: this.peakModelList() }
      return est
    } catch (err) {
  ...
  summaryAll() {
    ...
    for (const u of used) {
      ...
      if (isFree) freeCount++
      rows.push({ key: u.key, provider: u.provider, model: u.model, tokens: u.tokens, cost, currency, source, free: isFree })
    }
    ...
    return { rows, totals, tokens, totalTokens, hitRate, freeCount, at: new Date().toISOString(), hasUsage: used.length > 0 }
  }
