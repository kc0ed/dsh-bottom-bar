...
.dsh-peak-switch-btn:hover {
  border-color: var(--dsw-alias-brand-primary);
  color: var(--dsw-alias-brand-primary);
}
.dsh-peak-switch-name {
  flex: 1;
  text-align: center;
  font-weight: 600;
  font-size: 12px;
  ...
  border-top: 1px dashed var(--dsw-alias-border-l1);
          ...
          (props) => {
            ...
            const peakDetail = () => {
              ...
              if (modelList !== null && modelList.length > 1) {
                nodes.push(React.createElement('div', { className: 'dsh-peak-switch', key: 'sw' },
                  React.createElement('button', { className: 'dsh-peak-switch-btn', title: '上一个模型', onClick: () => setPeakModelIdx((curIdx - 1 + modelList.length) % modelList.length) }, '◀'),
                  React.createElement('span', { className: 'dsh-peak-switch-name' }, modelLabel(curModel.model)),
                  React.createElement('button', { className: 'dsh-peak-switch-btn', title: '下一个模型', onClick: () => setPeakModelIdx((curIdx + 1) % modelList.length) }, '▶'),
                ))
        ...
        ctx.slots.inject('settings.section', () => ctx.slots.register(
          ...
          () => {
            ...
            const optionsRef = React.useRef({ mode: 'separate', tooltip: 'auto', precision: 'compact', peakEnabled: false, peakRemind: true, peakTimezone: 'system' })
            ...
            optionsRef.current = { mode, tooltip: tooltipAlways ? 'always' : 'auto', precision, peakEnabled, peakRemind, peakTimezone: peakTz }
            React.useLayoutEffect(() => {
            ...
            return React.createElement('div', { className: 'dsh-comp-page', onDragOver: (e) => e.preventDefault() },
              ...
              React.createElement('p', { className: 'dsh-comp-desc' }, 'DeepSeek 自 2026-08-17 00:00 起实行峰谷定价：高峰时段 9:00-12:00 / 14:00-18:00（按所选时区，默认跟随系统），空闲时段为高峰一半。「计价」按高峰价算钱（仅官方 deepseek 渠道生效）；「提醒」只显示 ⏱ 高峰/空闲 与时段标注，不影响价格。'),
