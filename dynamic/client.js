  ...
  border-top: 1px dashed var(--dsw-alias-border-l1);
        ...
        ctx.slots.inject('settings.section', () => ctx.slots.register(
          ...
          () => {
            ...
            // 鼠标悬停条目时：预览区自动平滑滚动聚焦到对应高光分段；移开时自动平滑回弹
            React.useEffect(() => {
              ...
              if (hovered !== null && previewSegRefs.current[hovered]) {
                ...
                dock.scrollTo({ left: 0, behavior: 'smooth' })
              }
            }, [hovered])
            React.useEffect(() => {
              let cancelled = false
            ...
            const makeLineChildren = () => {
              ...
              previewSegs.forEach((item, i) => {
                ...
                children.push(React.createElement('span', {
                  ...
                  title: (SEGMENT_LABELS[item.id] || item.id) + ' · 悬停联动下方条目',
            ...
            return React.createElement('div', { className: 'dsh-comp-page', onDragOver: (e) => e.preventDefault() },
              ...
              React.createElement('p', { className: 'dsh-comp-desc' }, '配置输入框下方的底栏统计行。可点击左侧图标或按住 Ctrl 多选批量拖拽排序；开关组：显示/隐藏、输入缓存口径、黑条行为、费用精度；价格表在页面底部（折叠）。'),
